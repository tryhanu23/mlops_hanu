...
azure_databricks_resource_id="03485290-9c73-45af-afa2-9a235746bff3"
...
######################################################################################
#This command uses the curl command to send a POST request to the Microsoft Online Login service, 
#using the specified tenant ID, Azure Databricks resource ID, client ID, grant type, and client secret. 
#The response contains an access token, which is then extracted using the jq command and stored in the accessToken variable.
#This access token can then be used to authenticate requests to the Azure Databricks service.
